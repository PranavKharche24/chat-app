-- lua/actions.lua
function on_login(userID, password)
    send_command("LOGIN " .. userID .. " " .. password)
  end
  
  function on_register(username, email, dob, fullName, password)
    send_command(string.format("REGISTER %s %s %s %s %s",
      username, email, dob, fullName, password))
  end
  
  function on_send(recipient, msg)
    if recipient == "Group" then
      send_command("SENDALL " .. msg)
    else
      send_command("SEND " .. recipient .. " " .. msg)
    end
  end
  ...
  function on_create_group(name)
    send_command("CREATEGROUP " .. name)
  end
  package main
...
func main() {
  ...
  function on_join_group(name)
    send_command("JOINGROUP " .. name)
  end
  
  function on_leave_group(name)
    send_command("LEAVEGROUP " .. name)
  end
